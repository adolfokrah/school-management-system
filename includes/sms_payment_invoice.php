...
<?php 
 ...
 if(isset($_GET['qty'])&&!empty($_GET['qty'])&&isset($_GET['amount'])&&!empty($_GET['amount'])&&isset($_GET['unit_price'])&&!empty($_GET['unit_price'])){
     ...
     $operation = 'SMS';
     mysqli_query($conn,"INSERT INTO `payment_invoices` (`ID`, `SCHOOL`, `USER ID`,`USER NAME` ,`EMAIL`,`NUMBER`, `INVOICE NUMBER`, `DATE`, `OPERATION`, `UNIT PRICE`, `QUANTITY`, `STATUS`) VALUES (NULL, '$initials', '$user','$username','$useremail','$usernumber', '$in_number', '$date', 'SMS CREDIT', '$unit_price', '$qty','');");
...
function invoice_number($year,$conn){
    ...
    if($fetch = mysqli_fetch_assoc($qurery_pick)){
        ...
        $print = '';
        if($status == ""){
             $btn='<button type="button" class="btn btn-warning" onclick="submit_invoice(\''.$operation.'\',\''.$invoice_number.'\',\''.$id.'\');" id="invoice_btn"><i class="fa fa-visacard" ></i> Submit Invoice</button>';
            $title = "Please input the mobile number here.";
            $info = '<p class="text-muted well well-sm no-shadow" style="margin-top: 10px;">
          Money can  be sent to these numbers: <strong>MTN - 0245301631</strong>
