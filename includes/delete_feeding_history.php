...
include 'get_currencies.php';
    ...
    if(isset($_REQUEST['id']) &&isset($_REQUEST['student_id'])&&isset($_REQUEST['arreas'])&&isset($_REQUEST['balance'])&&isset($_REQUEST['days_left']) ){
        ...
        mysqli_query($conn,"update feeding_fee set `ARREAS`='$arreas',`BALANCE`='$balance',`DAYS LEFT`='$days_left' where `STUDENT ID`='$student_id'");
            
        //insert history
        $operation = 'Feeding fee payment of amount  '.$currency.' '.$amount.' Made by Student with ID '.$student_id.' ('.$student_name.') for '.$days.' day(s)  amount per day charged ='.$currency.'  '.sprintf('%0.2f',$amount_per_day).' deleted and student records set to<br/>
