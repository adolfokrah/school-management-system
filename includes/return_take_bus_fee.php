...
include 'get_currencies.php';
    ...
    if(isset($_REQUEST['ids'] )&& isset($_REQUEST['action'])){
        ...
        foreach($ids as $id){
            $query = mysqli_query($conn,"select * from `bus_fee` where `ID`='$id' ");
            ...
            $operation = '';
            if($action == "debitors"){
                 mysqli_query($conn,"update bus_fee set `ARREAS`='0' where `STUDENT ID`='$student_id'");
                $operation = 'BUS FEE ARREAS OF <strong>'.$currency.' '.$arreas.' </strong>taken from <strong>'.$student_name.'</strong> ID :'.$student_id;
            }else{
                 mysqli_query($conn,"update bus_fee set `BALANCE`='0' where `STUDENT ID`='$student_id'");
                 $operation = 'BUS FEE BALANCE OF <strong>'.$currency.' '.$balance.' </strong> given to <strong>'.$student_name.'/strong> ID :'.$student_id;
            }
