<?php
    ...
    if( isset($_REQUEST['student_id']) && isset($_REQUEST['row_id']) && isset($_REQUEST['from']) && isset($_REQUEST['to'])){
        ...
        $query = mysqli_query($conn,"select * from bus_fee where `STUDENT ID`='$student_id'");
        if($fetch = mysqli_fetch_assoc($query)){
            echo '<form role="form" id="form" enctype="multipart/form-data">
                                            ...
                                            <div class="form-group" >
                                                <button type="button" class="btn btn-block btn-warning"  onclick="delete_bus_fee(\''.$id.'\',\''.$from.'\',\''.$to.'\');" id="btn" >Confirm and delete</button>
