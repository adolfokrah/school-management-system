...
function add_row(action){
    if(action == 'fees'){
        var input_name = 'paid_by';
        var msg_placeholder = 'Payment by';
    }else if(action == 'feeding'){
        var input_name = 'feeding_date';
        var msg_placeholder = 'Days';
    }else{
        
    }
    var rows = $('#row_box').val();
    if(rows < 1){
        swal('Fail','Row should be greater than or equal to one ','error');
    }else{
        if(rows > 5){
           swal('Fail','Rows cannot exceed Five','error');
        }else{
            if(add_rows == 5){
                swal('Fail','Rows limit reached','error');
                if(add_rows - parseInt(rows) < 1){
                    swal('Fail','Rows cannot be less than one','error');
                }
            }else{
                if((add_rows + parseInt(rows)) > 5){
                    swal('Fail','Rows cannot exceed Five','error');
                }else{
                    add_rows = add_rows + parseInt(rows);
                    for(var i=0;i < rows;i++){
                        console.log(i);
                        $('#row_result_box').append('<li id="" class="row_list"><div class="form-group" style="padding:5px;"><div id="msg_box"></div><input type="text" class="form-control" placeholder="Student ID" name="student_id"/><input type="text" class="form-control" placeholder="Amount" name="p_amount"/><input type="text" class="form-control" placeholder="'+msg_placeholder+'" name="'+input_name+'"/> <button class="btn btn-danger"  type="button" id="button"><i class="fa fa-trash"></i></button></div></li>');
                    }
                    
                    if(add_rows > 0){
                        $('#make_payment').css('display','block');
                    }
                    var get_rows = document.getElementsByClassName('row_list');
                    for(var i=0; i<get_rows.length; i++){
                        get_rows[i].id = 'row_'+i;
                        var button = '#row_'+i+' #button'
                        $(button).attr('onclick','remove(\''+i+'\')');
                    }
                }
            }
        }
...
function make_payment(){
    var student_ids = document.getElementsByName('student_id');
    var amounts = document.getElementsByName('p_amount');
    var payers = document.getElementsByName('paid_by');
    var Payment_date = $('#pdate').val();
    var ids =[];
    var paid_amount = [];
    var ppayers =[];
    var row_id = [];
    ...
    for(var i=0; i<student_ids.length; i++){
        if(student_ids[i].value==""){
            student_ids[i].style.border="thin solid red";
            student_ids[i].value="";
            student_ids[i].placeholder="Student ID needed";
            insert = false;
            
        }else{
            if(insert == true){
                ids[i]=student_ids[i].value;
                insert = true;
            }
        }
    }
    ...
    for(var i=0; i<amounts.length; i++){
        if(amounts[i].value==""){
            amounts[i].style.border="thin solid red";
            amounts[i].value="";
            amounts[i].placeholder="Amount needed";
            insert = false;
            ...
            if(isNaN(amounts[i].value)){
                amounts[i].style.border="thin solid red";
                amounts[i].value="";
                amounts[i].placeholder="Amount must be in figures";
                insert = false;
            }else{
                if(insert == true){
                  paid_amount[i]=amounts[i].value;
                  insert = true;
                }
            }
    ...
    for(var i=0; i<payers.length; i++){
        if(payers[i].value==""){
            payers[i].style.border="thin solid red";
            payers[i].value="";
            payers[i].placeholder="Payment by";
            insert = false;
            
        }else{
            if(insert == true){
                ppayers[i]=payers[i].value;
                insert = true;
            }
        }
    }
    ...
    var get_rows = document.getElementsByClassName('row_list');
    for(var i=0; i<get_rows.length; i++){
        row_id[i] = get_rows[i].id ;
    }
    
    var check_btn = document.getElementById('pdate_check').checked;
    if(check_btn == true){
        if(Payment_date==""){
            insert = false;
            $('#pdate').css('outline','thin solid red');
            $('#pdate').attr('placeholder','Date required');
        }
    }else{
        Payment_date ="";
    }
   // alert(Payment_date);
    if(insert == true){
        $('#make_payment').html('<i class="fa fa-spinner fa-pulse fa-3x fa-fw" style="font-size:18px;"></i> Loading...');
        $('#make_payment').attr('disabled','true');
         $.post("../includes/pay_fees.php",{student_ids:ids,amounts:paid_amount,payers:ppayers,row_ids:row_id,date:Payment_date},
              function(data){
        ...
        $('#make_payment').removeAttr('disabled','true');
                    ...
                    for(var i=0; i<error.length; i++){
                        var id_index = error[i].indexOf('-');
                        var id = error[i].substr(0,id_index);
                        var message = error[i].substr(id_index + 1,error[i].length);
                        
                        if(message=="success"){
                           var print = true;
                            counter ++;
                            $('#'+id).css('box-shadow','0px 0px 2px 2px green');
                            $('#'+id+' #msg_box').css('color','green');
                            $('#'+id+' #msg_box').html(message);
                        }else if(message=="year"){
                            swal('Fail','Academic Year Not set','error');
                        }else{
                            $('#'+id).css('box-shadow','0px 0px 2px 2px red');
                            $('#'+id+' #msg_box').css('color','red');
                            $('#'+id+' #msg_box').html(message);    
                        }
                    }
                    if(print){
                        swal('Done',counter+' Payment(s) made','success');
...
function save_new_expens(){
    ...
    if( item != "" && unit_price !="" && date != "" && quantity !=""){
       if(insert == true){
        ...
        $.post("../includes/save_expens.php",{item:item,unit_price:unit_price,quantity:quantity,date:date,action:''},
              function(data){
            console.log(data);
...
function save_expens(){
    ...
    preConfirm:function(){
    return new Promise(function(resolve){
        $.post("../includes/save_expens.php",{item:'',unit_price:'',quantity:'',date:'',action:'save'},
              function(data){
            console.log(data);
            if(data ==='success'){
                swal({
...
var item = $('#edit_item_name').val();
    ...
    if( item != "" && unit_price !="" && date != "" && quantity !=""){
       if(insert == true){
        ...
        $.post("../includes/edit_expense.php",{item:item,unit_price:unit_price,quantity:quantity,date:date,id:id},
              function(data){
            console.log(data);
...
function update_school_info() {
    ...
    if (sch_id == "" || school_code == "" || school_name == "" || school_number == "" || sch_address == "" || principal_name == "") {
        swal(
            'Ooops..',
            'Some fields were left blank.',
            'error'
...
function submit_invoice(operation,invoice_number,id){
              ...
              function(data){
                ...
                $('#invoice_btn').removeAttr('disabled','true');
                 ...
                 if(data=="success"){
                     ...
                     window.open('sms_payment_invoice.php?id='+id,'_self');
                 }else{
...
function edit_arreas(id){
    ...
    var previous_arreas = parseFloat($('#parreas'+id).html());
    ...
    $('#pterm').val(previous_arreas);
    $('#nterm').val(previous_arreas);
            ...
            if (result.value) {
                ...
                $.ajax({
                  ...
                  contentType: false   // tell jQuery not to set contentType
                    ...
                    $('#add_student').removeAttr('disable','true');
                   console.log(data);
                   swal(data);
                });
                return false;
