<?php
    ...
    if(isset($_REQUEST['student_ids']) && isset($_REQUEST['amounts']) && isset($_REQUEST['pday']) && isset($_REQUEST['date']) && isset($_REQUEST['amount_per_day'])){  
        ...
        foreach($student_ids as $student_id){
            ...
            if($check > 1){
                ...
                if($fetch_student_details = mysqli_fetch_assoc($query_check)){
                    ...
                    $query_check_feeding_records = mysqli_query($conn,"select * from `feeding_fee` where `SCHOOL`='$initials' and `STUDENT ID`='$student_id'");
...
//                        }
                        ...
                        mysqli_query($conn,"update `feeding_fee` set `DAYS LEFT` = '$days_left',`BALANCE`='$balance',`ARREAS`='$arreas',`AMOUNT`='$amount' where `SCHOOL`='$initials' and `STUDENT ID`='$student_id'");
                        
                        
                        mysqli_query($conn,"INSERT INTO `daily_feeding_fee` (`ID`, `SCHOOL`,`AMOUNT PER DAY`,`AMOUNT`, `DATE`, `DAYS`, `STUDENT ID`, `STUDENT NAME`,`CLASS`,`ACADEMIC YEAR`, `TERM`) VALUES (NULL, '$initials','$amount_per_day', '".$amounts."', '$date', '".$pday."', '$student_id', '$student_name','$student_class','$academic_year', '$term');");
                        
                        
                        //insert history
                        $operation = 'Feeding fee payment of '.$date.' of amount  '.$currency.' '.$amounts.' Made for Student with ID '.$student_id.' ('.$student_name.') for '.$pday.' day(s)  amount per day charged ='.$currency.'  '.sprintf('%0.2f',$amount_per_day);
                        $date = date('Y-m-d');
                        ...
                        mysqli_query($conn,"INSERT INTO `feeding_fee` (`ID`, `SCHOOL`, `AMOUNT`,`ARREAS`,`BALANCE`, `DATE`, `DAYS LEFT`, `STUDENT ID`, `STUDENT NAME`, `ACADEMIC YEAR`, `TERM`) VALUES (NULL, '$initials', '".$amounts."','$arreas','$balance', '$date', '".($pday-1)."', '$student_id', '$student_name', '$academic_year', '$term');");
