...
function fetchdata($initials,$conn){
    ...
    if(isset($_GET['action']) && !empty($_GET['action']) || isset($_GET['academic_year']) || isset($_GET['class'])){
        $action = $_GET['action'];
        $fields = array();
       
        $topic = '';
        $topic2 ='';
        if($action=="admission_list"){
            $action = $_GET['academic_year'];
            $topic = "ADMISSION LIST "." <strong> (".$action.")</strong>" ;
           $topic2 = "ADMISSION LIST";
        }else{
             $academic_year = '';
             $term = '';
            ...
            $action = $_GET['class'];
            $topic="CLASS LIST"." (".$academic_year.")";
        }
            $school_logo = '';
            $school_name = '';
            $school_moto='';
            ...
            $output = '<table cellpadding="5px;" style="border:0.5 dashed #000">
                    <tbody> 
                        ...
                        <tr>
                            ...
                            <td style="border:0.5px solid #000; font-size:13px;; text-align:center; width:100%"><strong>'.$topic.'</strong></td>
                        ...
                        <tr>
                            ...
                            if($topic2 != ""){
                                $output .= '<td style="border:0.5px solid #000; font-size:13px;; width:20%"><strong>CLASS ADMITTED</strong></td></tr>';
                            }else{
                                 $output .= '<td style="border:0.5px solid #000; font-size:13px;; width:20%"><strong>CLASS</strong></td></tr>';
                            }
        ...
        $query = '';
        if($topic2==""){
            $query = mysqli_query($conn,"select * from `admitted_students` where `PRESENT CLASS` = '$action' and `SCHOOL`='$initials' or `ACADEMIC YEAR` = '$action' and `SCHOOL`='$initials' order by `STUDENT LAST NAME` asc");
        }else{
            $query = mysqli_query($conn,"select * from `admitted_students` where `PRESENT CLASS` = '$action' and `SCHOOL`='$initials' or `ACADEMIC YEAR` = '$action' and `SCHOOL`='$initials' order by `STUDENT LAST NAME` and `CLASS ADMITTED` asc");
        }
        while($fetch = mysqli_fetch_assoc($query)){
            ...
            if($topic2 != ""){
                $fields[3]=htmlentities($fetch['CLASS ADMITTED']);
            }else{
                $fields[3]=htmlentities($fetch['PRESENT CLASS']);
            }
            $counter ++;
            ...
            $output .='
                        ...
                        <tr>
                            ...
                            <td style="border:0.5px solid #000; font-size:13px;; width:20%">'.$fields[3].'</td>
                        </tr>
                    ';
...
$pdf->SetTitle('STUDENT LIST SHEET');
