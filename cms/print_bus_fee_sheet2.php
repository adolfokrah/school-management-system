...
function fetchdata($initials,$conn){
    ...
    if(isset($_GET['class']) && !empty($_GET['class'])&&isset($_GET['group']) && !empty($_GET['group'])){
            ...
            $output = '<table cellpadding="5px;" style="border:0.5 dashed #000">
                    <tbody> 
                        ...
                        <tr>
                            ...
                            <td style="border:0.5px solid #000; font-size:13px;; text-align:center; width:100%"><strong>BUS FEE PAYMENT SHEET - '.$action.'</strong></td>
        ...
        while($fetch = mysqli_fetch_assoc($query)){
            ...
            if($action == "debitors"){
                     $query_pick_days_left = mysqli_query($conn,"select * from bus_fee where ARREAS > 0 and `SCHOOL`='$initials' and `STUDENT ID`='".$fields[2]."'");
                }else{
                    $query_pick_days_left = mysqli_query($conn,"select * from bus_fee where  `SCHOOL`='$initials' and `BALANCE`>0 and `STUDENT ID`='".$fields[2]."'");
