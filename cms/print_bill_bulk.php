...
function fetchdata($initials,$conn,$student_id){
...
$pdf->SetFont('times', '', 12);
 ...
 if(isset($_GET['class']) && !empty($_GET['class'])){
        $class = $_GET['class'];
         $query1 = mysqli_query($conn,"select * from admitted_students where `SCHOOL`='$initials'   and `PRESENT CLASS`='$class' order by `STUDENT LAST NAME` asc");
        ...
        while($fetch = mysqli_fetch_assoc($query1)){
            ...
            // Print a text
            $html = fetchdata($initials,$conn,$student_id);
