...
<?php include '../includes/cms_header.php';
            ...
            <div id="content">
                ...
                <div id="box">
                    ...
                    <div class="content" style="padding:20px; padding-top:0px;">
                        ...
                        $feeding_group = '';     
                        $initial_class = '';
                        if(isset($_GET['feeding']) && !empty($_GET['feeding']) && isset($_GET['class']) && !empty($_GET['class'])){
                            $feeding_group = $_GET['feeding'];
                            $initial_class = $_GET['class'];
                            
                            if($feeding_group == "debitors"){
                                echo '<div class="col-xs-12 col-sm-4" style="padding-top:20px; padding-bottom:50px;">
                            
                           
                             <button type="button" class="btn btn-primary" id="Continue" onclick="take_feeding(\''.$feeding_group.'\',\''.$initial_class.'\')">Take Arreas</button>
                        </div>';
                            }else{
                                echo '<div class="col-xs-12 col-sm-4" style="padding-top:20px; padding-bottom:50px;">
                            
                           
                             <button type="button" class="btn btn-primary" id="Continue" onclick="take_feeding(\''.$feeding_group.'\',\''.$initial_class.'\')">Return Balance</button>
                        </div>';
                            }
                        }
                        ?>
                        ...
                        <div class="col-sm-12">
                            <form action="feeding_fee_debitors_creditors" method="get" class="form-inline">
                            <div class="form-group"><label><small>Filter by Class </small></label><br/>
                           <select class="selectpicker" data-show-subtext="true" data-live-search="true" style="width:100%;" id="class" name="feeding">
                           ...
                           <button type="submit" class="btn btn-default" style="margin-top:25px;">Search</button>
                              <a href="print_feeding_fee_sheet2?group=<?php echo $feeding_group?>&class=<?php echo $initial_class;?>" target="_blank"><button type="button" class="btn btn-danger pull-right" id="print_feeding_sheet_btn" ><i class="fa fa-print"></i> Generate Report</button></a>
                        </form>
                        </div>
                        <div class="content">
                            ...
                            <div class="panel panel-default">
                                <div class="panel-heading">
                                    <span style="color:#6b6b6b">CMS / Feeding Fee /</span> <span style="color:#3c8dbc"> - <strong><?php echo $feeding_group;?> -  <?php echo $initial_class;?></strong></span>
                                </div>
                            </div>
    ...
    $query1 ='';
    if($feeding_group == "debitors"){
        $query1 = mysqli_query($conn,"select * from `feeding_fee` where `SCHOOL`='$initials' and `ARREAS`>'0' order by `STUDENT NAME` asc");
    }else{
        $query1 = mysqli_query($conn,"select * from `feeding_fee` where `SCHOOL`='$initials' and `BALANCE`>'0' order by `STUDENT NAME` asc");
    }
    ...
    while($fetch1 = mysqli_fetch_assoc($query1)){
        ...
        $arreas = $fetch1['ARREAS'];
        $balance = $fetch1['BALANCE'];
        $counter ++;
        $id = $fetch1['ID'];
        $query2 = mysqli_query($conn,"select * from admitted_students where `ADMISSION NO / ID`='$student_id' and `PRESENT CLASS`='$initial_class'");
        ...
        <script src="../js/webcam_config.js"></script>
         <script type="text/javascript">
             $(document).ready(function () {
                 ...
                 $('.list-unstyled li:nth-child(10) ul li:nth-child(3)').toggleClass('active');
             });
