                    ...
                    <div class="content" style="padding:20px; padding-top:0px;">
                        ...
                        if(isset($_GET['class']) && !empty($_GET['class'])){
                            $initial_class = $_GET['class'];
                                ...
                                $initial_class = '';
                                $query_pick_initial_class="";
                                if($username == 'Administrator' || $username == 'Data Entry'){
                                 $query_pick_initial_class = mysqli_query($conn,"select * from subjects where SCHOOL='".$initials."' order by id asc");
                                 $initial_class = htmlentities($fetch['CLASS']);
                                }else{
                                $query_pick_initial_class = mysqli_query($conn,"select * from subjects where TEACHER ='$user' and SCHOOL='".$initials."' order by id asc");}
                        ...
                        ?>
                            ...
                            <div class="panel panel-default">
                                <div class="panel-heading">
                                    <span style="color:#6b6b6b">CMS / Student /</span> <span style="color:#3c8dbc">Marksheet - <strong><?php echo $initial_class;?></strong></span>
                                </div>
                            </div>
                        ...
                        <div class="col-sm-12">
                            <form action="manage_marksheet.php" method="get">
                           ...
                           <select class="selectpicker" data-show-subtext="true" data-live-search="true" style="width:100%;" id="class" name="class">
                            <?php
                                    ...
                                    while($fetch = mysqli_fetch_assoc($qeuery_pick_classes)){
                                       if($username == 'Administrator' || $username == 'Data Entry'){
                                        if(mysqli_num_rows(mysqli_query($conn,"select * from subjects where CLASS='".$fetch['CLASS']."' order by id asc")) !=null){
                                        
                                        echo '<option value="'.htmlentities($fetch['CLASS']).'" >'.htmlentities($fetch['CLASS']).'</option>';   
                                    }} else{
                                    if(mysqli_num_rows(mysqli_query($conn,"select * from subjects where TEACHER ='$user' and CLASS='".$fetch['CLASS']."' order by id asc")) !=null){
                           ...
                           <button type="submit" class="btn btn-default">Search</button>
                            ...
                            <form action="edit_m_subject" method="get">
                                 <label><small>edit by subject </small></label><br/>
                           <select class="selectpicker" data-show-subtext="true" data-live-search="true" style="width:100%;" id="class" name="subject">
                            <?php
                                   if($username == 'Administrator' || $username == 'Data Entry'){
                                    $qeuery_pick_classes = mysqli_query($conn,"select * from subjects where `SCHOOL`='$initials' and `CLASS`='$initial_class' order by ID asc");
                                    while($fetch = mysqli_fetch_assoc($qeuery_pick_classes)){
                                        echo '<option value="'.htmlentities($fetch['SUBJECT NAME']).'" >'.htmlentities($fetch['SUBJECT NAME']).'</option>';
                                    }}else{
                                    $qeuery_pick_classes = mysqli_query($conn,"select * from subjects where `SCHOOL`='$initials' and `CLASS`='$initial_class' and TEACHER ='$user' order by ID asc");
                                    while($fetch = mysqli_fetch_assoc($qeuery_pick_classes)){
                                        echo '<option value="'.htmlentities($fetch['SUBJECT NAME']).'" >'.htmlentities($fetch['SUBJECT NAME']).'</option>';
                                    }}
                            ...
                            <input type="hidden" value="<?php echo $initial_class?>"name="class"/>
    ...
    while($fetch1 = mysqli_fetch_assoc($query1)){
        ...
        echo '<tr id="row'.$id.'">
              ...
              <td style="width:30%;">
                ...
                <button class="btn btn-default" id="drop" data-toggle="modal" data-target="#vmarksheet" onclick="fetch_student_marks(\''.$student_id.'\',\''.$initial_class.'\');"><i class="fa fa-book"> </i> View</button>
