...
<html>
    <head>
        ...
        <title>Easyskul - CMS::Unsubmitted Invoices</title>
...
?>                       
                           ...
                           <div class="content">
                            ...
                            <div class="panel panel-default">
                                <div class="panel-heading">
                                    <span style="color:#6b6b6b">CMS / SUBMITTED STUDENTS</span>
                                </div>
                            </div>
    ...
    $query1 = mysqli_query($conn,"select * from payment_invoices where `SCHOOL`='$initials'  and `STATUS` ='' order by `DATE` asc");
    ...
    while($fetch1 = mysqli_fetch_assoc($query1)){
        ...
        $userid = htmlentities($fetch1['USER ID']);
        $mobile_numer = htmlentities($fetch1['MOBILE MONEY NUMBER']);
        $invoice_number = htmlentities($fetch1['INVOICE NUMBER']);
        $date = $fetch1['DATE'];
        $unit_price = $fetch1['UNIT PRICE'];
        $qty = $fetch1['QUANTITY'];
        $status = $fetch1['STATUS'];
        $operation = $fetch1['OPERATION'];
        ...
        echo '<tr id="row'.$id.'" '.$style.'>
              ...
              <div class="dropdown" >    
                <button class="btn btn-default"  data-toggle="dropdown" id="drop">Action <span class="caret"></span></button>
                    <ul class="dropdown-menu" id="dropdown" class="dropdown" style="color:black"> 
                        <a href="sms_payment_invoice.php?id='.$id.'" target="_blank"><li>View</li></a>
                        <li onclick="delete_invoice(\''.$id.'\',\'unsubmitted\')">Deleted</li>
                    </ul>
              </div>
        ...
        <script src="../js/webcam_config.js"></script>
         <script type="text/javascript">
             $(document).ready(function () {
                 ...
                 $('.list-unstyled li:nth-child(24) ul').attr('aria-expanded','true');
                 $('.list-unstyled li:nth-child(24) a').attr('aria-expanded','true');
                 $('.list-unstyled li:nth-child(24) ul').toggleClass('collapse in');
                 $('.list-unstyled li:nth-child(24) a').toggleClass('');
                 $('.list-unstyled li:nth-child(24) ul li:last-child').toggleClass('active');
             });
