...
include '../includes/get_currencies.php';
    if(isset($_GET['class']) && isset($_GET['from']) && isset($_GET['to']) && isset($_GET['group'])){
        $class = $_GET['class'];
        $from = $_GET['from'];
        $to = $_GET['to'];
       $group = $_GET['group'];
       ...
       $group = str_replace('_','&',$group);
            ...
            $output = '<table cellpadding="5px;" style="border:0.5 dashed #000">
                    <tbody> 
                            ...
                            <td style="border:0.5px solid #000; font-size:13px;; text-align:center; width:100%"><strong>BUS FEE PAYMENT REPORT</strong></td>
                            ...
                            <td style="border:0.5px solid #000; font-size:13px;; width:20%"><strong>STUDENT NAME</strong></td>
                            <td style="border:0.5px solid #000; font-size:13px;; width:10%"><strong>PAID FOR</strong></td>
                        ...
                        if($class == ''){
                            if($group == "ALL"){
                                $query2 = mysqli_query($conn,"select * from daily_bus_fee where `DATE` between '$from' and '$to' and `SCHOOL`= '$initials' order by  `TERM` and `CLASS` and `STUDENT NAME` asc");
                            }else{
                                $query2 = mysqli_query($conn,"select * from daily_bus_fee where `DATE` between '$from' and '$to' and `SCHOOL`= '$initials' and `CATEGORY`='$group' order by  `TERM` and `CLASS` and `STUDENT NAME` asc");
                            }
                            
                        }else{
                            if($group == "ALL"){
                                $query2 =  mysqli_query($conn,"select * from daily_bus_fee where `DATE` between '$from' and '$to' and `SCHOOL`= '$initials' and `CLASS`= '$class' order by  `TERM` and `STUDENT NAME` asc");
                            }else{
                                $query2 =  mysqli_query($conn,"select * from daily_bus_fee where `DATE` between '$from' and '$to' and `SCHOOL`= '$initials' and `CLASS`= '$class' and `CATEGORY`='$group' order by  `TERM` and `STUDENT NAME` asc");
                            }
          ...
          $total_amount=0;
            while($fetch = mysqli_fetch_assoc($query2)){
                ...
                $output .='<tr>
                            ...
                            <td style="border:0.5px solid #000; font-size:13px;; width:20%">'.htmlentities($fetch['STUDENT NAME']).'</td>
                             <td style="border:0.5px solid #000; font-size:13px;; width:10%">'.htmlentities($fetch['CATEGORY']).'</td>
...
$pdf->SetTitle('BUS FEE HISTORY SHEET');
...
$pdf->Output('BUS FEE PAYMENT HISTORY.pdf', 'I');
