...
<div id="box">
                ...
                <div class="content">
                    ...
                    $query_select2 = mysqli_query($conn,"select * from admitted_students where `SCHOOL`='$initials' and `PRESENT CLASS` !=''");
...
//                                        }
                                ...
                                <div class="content">
                                    <div class="col-sm-12 col-md-5">
                                        <div class="panel-group">
                                            <div class="panel panel-default">
                                                <div class="panel-heading">
                                                    <span style="color:#6b6b6b"><i class="fa fa-money"></i> Today's Accounting</span>
                                                </div>
                                            </div>
                                        </div>
                                        <?php
                                            //sumarize accounting amount
                                            
                                            //get school fees
                                            $get_current_date = date('Y-m-d');
                                            $query_select_school_fees = mysqli_query($conn,"select * from `daily_fees_payments` where `SCHOOL`='$initials' and `DATE`='$get_current_date'");
                                            $school_fees=0;
                                            while($fetch_school_fees = mysqli_fetch_assoc($query_select_school_fees)){
                                                $school_fees = $school_fees + $fetch_school_fees['AMOUNT PAID'];
                                            }
                                            
                                            //convert school fees amont to tow decimal place
                                            $school_fees= sprintf('%0.2f',$school_fees);
                                        
                                            //select feeding fees
                                            $query_select_feeding_fee = mysqli_query($conn,"select * from feeding_fee where `SCHOOL`='$initials' and `DATE`='$get_current_date'");
                                            $feeding_fee=0;
                                            while($fetch_feeding_fee = mysqli_fetch_assoc($query_select_feeding_fee)){
                                                $feeding_fee = $feeding_fee + $fetch_feeding_fee['AMOUNT'];
                                            }
                                            
                                            //convert school fees amont to tow decimal place
                                            $feeding_fee= sprintf('%0.2f',$feeding_fee);
                                        
                                            //select transport fees
                                            $query_select_transport_fee = mysqli_query($conn,"select * from daily_bus_fee where `SCHOOL`='$initials' and `DATE`='$get_current_date'");
                                            $transport_fee=0;
                                            while($fetch_transport_fee = mysqli_fetch_assoc($query_select_transport_fee)){
                                                $transport_fee = $transport_fee + $fetch_transport_fee['AMOUNT'];
                                            }
                                            
                                            //convert school fees amont to tow decimal place
                                            $transport_fee= sprintf('%0.2f',$transport_fee);
                                        
                                            //select expenses
                                            $query_select_expenses = mysqli_query($conn,"select * from expenses where `SCHOOL`='$initials' and `DATE`='$get_current_date'");
                                            $expenses=0;
                                            while($fetch_expenses = mysqli_fetch_assoc($query_select_expenses)){
                                                $amount = $fetch_expenses['UNIT PRICE'] * $fetch_expenses['QUANTITY'];
                                                $expenses = $expenses + $amount;
                                            }
                                            
                                            //convert school fees amont to tow decimal place
                                            $expenses= sprintf('%0.2f',$expenses);
                                        ?>
                                        <div class="panel-group">
                                        ...
                                        <div class="panel-group">
                                            <div class="panel panel-warning">
                                                <div class="panel-heading">
                                                    Feeding Fee Collected
                                                </div>
                                                <div class="panel-body">
                                                    <?php echo $currency.' '.$feeding_fee; ?>
                                                </div>
                                        ...
                                        <div class="panel-group">
                                            <div class="panel panel-danger">
                                                ...
                                                <div class="panel-body">
                                                    <?php echo $currency.' '.$transport_fee; ?>
                                                </div>
                                              ...
                                              <div class="progress">
                                                ...
                                                <?php 
                                                    ...
                                                    if(mysqli_num_rows($query_pick)!=null){
                                                        $off = 'checked';    
                                                        $info = 'Check to put sms on';
                                                    }
                                                    echo '<input type="checkbox" id="sms_active" '.$off.'/> <span id="sms_info">'.$info.'</span>';
                                                ?>
   ...
   $(document).ready(function() {
             ...
             $('#sms_active').on('click',function(){
                var checkbox = document.getElementById('sms_active');
                 $('#sms_info').html('<i class="fa fa-spinner fa-pulse fa-3x fa-fw" style="font-size:13px;;"></i> Please wait...');
                 $.post("../includes/activate_sms2.php", {
         
                    },function(data){
                     if(checkbox.checked){
                     $('#sms_info').html('Check to put sms on');
                     }else{
                         $('#sms_info').html('Check to put sms off');
                     }
                 })
             })
         </script>
